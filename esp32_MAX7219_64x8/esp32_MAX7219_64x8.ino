...
//        P.setZoneEffect(ZONE_RIGHT, true, PA_FLIP_LR);
        ...
        P.displayZoneText(ZONE_LEFT, szTimeL, PA_CENTER, 0, 0, PA_PRINT, PA_NO_EFFECT);
        if (millis() - ulLastShowClockTime >= 1000) {
            ulLastShowClockTime = millis();
            getTime1();       
            P.displayReset(ZONE_LEFT);
        }
                
        if (P.getZoneStatus(ZONE_RIGHT)){
            switch (msgIdx){
            case 0:                
                scroll_speed = 80;
            ...
            if(msgIdx++ >= 2) { msgIdx = 0; }    
            
            // P.displayClear();
            // P.synchZoneStart();
            P.displayReset(ZONE_RIGHT);
        }  
