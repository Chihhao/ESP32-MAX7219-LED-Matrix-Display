...
### 網頁設定介面
![setting-page](Images/web-setting-page.png)
...
### ⚠️ 編譯常見問題與修正 (Compilation Troubleshooting)

如果您在編譯過程中遇到錯誤，請參考以下修正方式：

1.  **錯誤：`narrowing conversion of '49828' from 'int' to 'uint8_t'`**
    *   **原因**：`PS2Keyboard` 程式庫 (v2.4) 原始碼包含特殊符號 `'¤'`，在新版編譯器中會被視為多位元組整數而報錯。
    *   **解法**：需手動修改程式庫檔案 `PS2Keyboard.cpp` (通常位於 `Documents/Arduino/libraries/PS2Keyboard/PS2Keyboard.cpp`)。
        *   開啟檔案，找到第 265 行附近的程式碼：`0 /*CapsLock*/, 0 /*Rshift*/, PS2_ENTER /*Enter*/, '¤', 0, '#', 0, 0,`
        *   將 `'¤'` 替換為 `0xA4`：`0 /*CapsLock*/, 0 /*Rshift*/, PS2_ENTER /*Enter*/, 0xA4, 0, '#', 0, 0,`

2.  **錯誤：`'class PS2Keyboard' has no member named 'clear'`**
    *   **原因**：標準版 `PS2Keyboard` v2.4 不包含 `clear()` 函式。
    *   **解法**：本專案程式碼已針對此問題進行修正 (使用 `while(keyboard.available()) keyboard.read();` 取代)。若您使用舊版程式碼或自行撰寫，請留意此差異。
...
3.  **🌐 網頁控制**:
    *   連線成功後，手機或電腦通常會自動跳出設定頁面 (Captive Portal)。
    *   若未自動跳出，請開啟瀏覽器輸入網址 `http://192.168.4.1`。
    *   **設定介面說明**:  
        *   **開關 (08:30 ~ 17:30)**: 手動開啟或關閉顯示器 (預設依據工作時間自動開關)。
        *   **旋轉 180 度**: 若安裝方向相反，可開啟此選項將畫面翻轉。
        *   **訊息欄位 (1~3)**: 輸入欲輪播的英文訊息。
        *   **傳送文字**: 將輸入的訊息更新至顯示器。
        *   **重置預設文字**: 恢復預設的歡迎訊息。
        *   **校正時間**: 將 ESP32 RTC 時間同步為目前手機/電腦的時間。
4.  **🔢 條碼輸入**:
